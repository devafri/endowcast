...
const filteredAndSortedSimulations = computed(() => {
  let filtered = savedSimulations.value;
  ...
  if (searchQuery.value.trim()) {
    ...
    filtered = filtered.filter(sim => 
      ...
      (sim.portfolio?.name || '').toLowerCase().includes(query)
    );
  }

  // Apply status filter
  if (filterStatus.value === 'completed') {
    filtered = filtered.filter(sim => sim.isCompleted);
...
const selectedSimulations = computed(() => {
  return savedSimulations.value.filter(sim => selectedForComparison.value.has(sim.id));
});
...
function toggleSimulationSelection(simulationId: string) {
  if (selectedForComparison.value.has(simulationId)) {
    selectedForComparison.value.delete(simulationId);
  } else {
    // Limit to 4 simulations for comparison
    if (selectedForComparison.value.size < 4) {
      selectedForComparison.value.add(simulationId);
    }
  }
}
  ...
  <main class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="card p-6">
          ...
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">
                {{ filteredAndSortedSimulations.length }} of {{ savedSimulations.length }} simulation{{ savedSimulations.length !== 1 ? 's' : '' }}
        ...
        <div v-if="!isLoadingHistory && savedSimulations.length > 0" class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div v-if="searchQuery || filterStatus !== 'all' || sortBy !== 'updated' || sortDirection !== 'desc'" class="mt-4 flex justify-between items-center">
            <span class="text-sm text-gray-600">
              {{ filteredAndSortedSimulations.length }} simulation{{ filteredAndSortedSimulations.length !== 1 ? 's' : '' }} found
            </span>
        ...
        <div v-else-if="filteredAndSortedSimulations.length === 0" class="text-center py-12">
        ...
        <div v-else class="space-y-6">
          <div v-for="simulation in filteredAndSortedSimulations" :key="simulation.id" class="mb-6 border border-gray-200 rounded-lg overflow-hidden"
               :class="[
               ...
               @click="comparisonMode ? toggleSimulationSelection(simulation.id) : null"
          >
            <!-- Simulation Header -->
    ...
    <div v-if="showComparisonModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click="showComparisonModal = false">
      <div class="bg-white rounded-lg max-w-7xl w-full max-h-[90vh] overflow-hidden" @click.stop>
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          <div class="grid gap-6" :class="`grid-cols-${Math.min(selectedSimulations.length, 2)}`">
            <div v-for="simulation in selectedSimulations" :key="simulation.id" class="border border-gray-200 rounded-lg p-4">
              <!-- Simulation Header -->
