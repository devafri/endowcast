...
// Use sim.options.years as the source of truth, fallback to results if needed
const sim = useSimulationStore();
const years = sim.options.years
  ?? props.results?.yearLabels?.length
  ?? props.results?.simulations?.[0]?.length
  ?? 10;
