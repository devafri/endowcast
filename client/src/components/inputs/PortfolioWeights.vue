...
<template>
  ...
  <div>
    <div class="flex justify-between items-center mb-4">
      <p class="text-sm text-text-secondary">Adjust the sliders to set your target allocation (must total 100%).</p>
      <div class="flex items-center space-x-3">
        <div class="hidden sm:flex items-center space-x-2 mr-4">
          <button type="button" class="btn-secondary py-1 px-3 text-sm" @click="applyPreset('Conservative')">Conservative</button>
          <button type="button" class="btn-secondary py-1 px-3 text-sm" @click="applyPreset('Balanced')">Balanced</button>
          <button type="button" class="btn-secondary py-1 px-3 text-sm" @click="applyPreset('Growth')">Growth</button>
        </div>
  <button type="button" class="btn-secondary py-1 px-3 text-sm" @click="rebalanceTo100">Rebalance</button>
        <button type="button" class="btn-secondary py-1 px-3 text-sm" @click="resetToDefaults">Reset</button>
        <div class="text-right">
        <div
          class="text-lg font-bold"
          :class="Math.round(total) === 100 ? 'text-accent' : 'text-warning'"
          aria-live="polite"
        >
          {{ total.toFixed(1) }}%
        </div>
        <div class="text-xs text-text-secondary">Total Allocation</div>
        </div>
      </div>
    </div>

  <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
      <div v-for="c in categoryList" :key="c.key" class="space-y-2">
        <div class="flex items-center justify-between">
          <span class="text-sm font-medium text-text-primary flex items-center">
            <input type="checkbox" v-model="locked[c.key]" class="mr-2" :aria-label="`Lock ${c.label}`" />
            <span :class="c.locked ? 'opacity-60' : ''">{{ c.label }}</span>
          </span>
          <div class="flex items-center space-x-2">
            <span class="badge">min {{ c.min ?? 0 }}%</span>
            <span class="badge">max {{ c.max ?? 100 }}%</span>
            <span class="value-pill">{{ (weights[c.key] ?? 0).toFixed(0) }}%</span>
          </div>
        </div>
        <input
          type="range"
